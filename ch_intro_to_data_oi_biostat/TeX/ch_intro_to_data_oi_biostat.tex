...
Individual-level data from the study are shown in Table \ref{leapStudyResultsDF}, for 5 of the 530 children -- each row represents a participant, and shows the participant's study ID number, treatment group assignment, and OFC outcome. 
...
\begin{example}{On the log-transformed scale, mean $\log$ income is 8.86, with standard deviation 1.33. Apply the empirical rule to describe the distribution of average yearly per capita income among the 248 countries.}

According to the empirical rule, the middle 70\% of the data are within one standard deviation of the mean, in the range (8.86 - 1.33, 8.66 + 1.33) = (7.53, 10.19) log(USD). 95\% of the data are within two standard deviations of the mean, in the range (8.86 - 2(1.33), 8.66 + 2(1.33)) = (6.20, 11.52) log(USD). 	

Undo the log transformation. The middle 70\% of the data are within the range ($e^{7.53}$, $e^{10.19}$) = (\$1,863, \$26,635). The middle 95\% of the data are within the range ($e^{6.20}$, $e^{11.52}$) = (\$493, \$100,710).	
\end{example}
...
\begin{example}{Body mass index (BMI) is a measure of weight commonly used by health agencies to assess whether someone is overweight, and is calculated from height and weight.\footnote{$BMI = \dfrac{weight_{kg}}{height^{2}_m} = \dfrac{weight_{lb}}{height^{2}_{in}} \times 703$} Describe the relationships shown in Figure~\ref{famussHeightWeightBmi}. Why is it helpful to use BMI as a measure of obesity, rather than weight?
	...
	\begin{figure}[h!]
		...
		\subfigure[]{
			\includegraphics[width=0.7\textwidth]
			{ch_intro_to_data_oi_biostat/figures/famussHeightBmi/famussHeightBmi}
			\label{famussHeightBmi}
		}
		\caption{\subref{famussHeightWeight} A scatterplot showing height versus weight from the \data{famuss} data. One participant 70.5 inches tall and weighing 308 pounds is highlighted. \subref{famussHeightBmi} A scatterplot showing height versus BMI from the \data{famuss} data. The same individual highlighted in \subref{famussHeightWeight} is marked here, with BMI 43.56.}
		\label{famussHeightWeightBmi}
	\end{figure}	
	
}

Figure~\ref{famussHeightWeight}	shows a positive association between height and weight; taller individuals tend to be heavier. Figure~\ref{famussHeightBmi} shows that height and BMI do not seem to be associated; the range of BMI values observed is roughly consistent across height. 
...
\begin{example}{Figure~\ref{incomeLifeExpectancy} is a scatterplot of life expectancy versus annual per capita income for 248 countries in 2012. Life expectancy is measured as the expected lifespan for children born in 2012 and income is adjusted for purchasing power in a country. Describe the relationship between life expectancy and annual per capita income; do they seem to be linearly associated?
		
		\begin{figure}[h]
			\centering
			\includegraphics[width=0.8\textwidth]
			{ch_intro_to_data_oi_biostat/figures/incomeLifeExpectancy/incomeLifeExpectancy.pdf}
			\caption{A scatterplot of life expectancy (years) versus annual per capita income (US dollars).}
...
Figure~\ref{incomeLifeExpectancyLog} shows an approximately linear relationship; a correlation coefficient is a reasonable numerical summary of the relationship. As calculated from statistical software, $r = 0.77$, which is indicative of a strong linear relationship.
