...
%JV: removing mention of use of the model for 'predictions', also not generally necessary to make predictions about cholesterol from a practical perspective
...
%If the goal is to make predictions, then a larger model might be preferable even if it includes variables with slope coefficients that are not statistically significant. 
...
%The model will also be assessed with respect to its reliability for making predictions about bird abundance based on features of a patch.
...
%JV: Discussion about a prediction model from these data should be in Notes, along with mentioning importance of validation, etc.
...
%DH: My defense of not using Bonferroni in regression is weak, and may simply be wrong.  A substantial fix would be to go back to all the regression section and at least point out that Bonferroni should be used more often.  I don't like that idea either, though.  I have a contradiction in the notes section.
...
\begin{description}
	...
	\item \emph{Critically examine residual plots.} All models are approximations, so it is not necessary to be concerned about relatively minor violations of assumptions; residual plots are seldom as well behaved as those for the PREVEND data. In some cases, like with the California data, residual plots show obvious major violations. With intermediate cases such as in the \data{forest.birds} plots, examine the plots closely and provide a detailed assessment of where the model seems less reliable.
