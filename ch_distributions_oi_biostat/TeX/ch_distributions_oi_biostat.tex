...
\begin{termBox}{\tBoxTitle{Negative binomial distribution}
		...
		A negative binomial random variable $X$ can be expressed as $X \sim \textrm{NB}(k, p)$.
...
$\textrm{NB}(k, p)$\vspace{1mm}\\\footnotesize Neg. Bin. dist.\\with $k$ successes\\\& $p$ prob. of success]{\raggedright\vspace{-5mm}
	
	$\textrm{NB}(k, p)$\vspace{1mm}\\\footnotesize Neg. Bin. dist.\\with $k$ successes\\\& $p$ prob. of success} 
...
The probability of a single success is $p=0.8$, the number of successes is $k=4$, and the number of necessary attempts under this scenario is $n=5$.
\begin{align*}
...
\begin{exercise}
Assume that each extraction attempt is independent. What is the probability that the fourth success occurs within 5 attempts?\footnote{If the fourth success ($k=4$) is within five attempts, it either took four or five tries ($n=4$ or $n=5$):
\begin{align*}
...
In \textsf{R}, the negative binomial distribution is defined as the number of failures that occur before a target number of successes is reached; i.e., $n - k$. In this text, the distribution is defined in terms of the total number of trials required to observe $k$ successes, where the last trial is necessarily a success. 
